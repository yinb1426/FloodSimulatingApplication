 ...
 *     6.  waterHeight：水面高度
 *	   7.  surfaceHeight：地表高度
 *     8.  damHeight：堤坝高度
 *     9.  drainRate：雨水井上下水速率
 * 	   10. gTerrainHeight：地形高度（9-12为存放在GPU显存中的对应数据，与5-8一一对应）
 * 	   11. gWaterHeight：水面高度
 *     12. gSurfaceHeight：地表高度
 * 	   13. gDamHeight：堤坝高度
 * 	   14. gDrainRate：雨水井上下水速率
 */
...
protected:
	...
	vector<double> surfaceHeight;						// 地表高度
	...
	double* gSurfaceHeight = nullptr;					// 地表高度(GPU)
...
public:
	Model(size_t _sizeX, size_t _sizeY, double _deltaX, double _deltaT, vector<double> _terrainHeight, vector<double> _waterHeight, vector<double> _damHeight, vector<double> _drainRate)
		: sizeX(_sizeX), sizeY(_sizeY), deltaX(_deltaX), deltaT(_deltaT), terrainHeight(_terrainHeight), waterHeight(_waterHeight), surfaceHeight(_terrainHeight), damHeight(_damHeight), drainRate(_drainRate)
	{}
	...
	void UpdateSurfaceHeightWithDamHeight();
	vector<double> GetSurfaceHeight() const;
